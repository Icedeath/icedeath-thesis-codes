...
for i=1:N_samples_m
    ...
    fcc = unifrnd (fc_min, fc_max,size(class_i,2),1)*fc;
    ...
    for j =1:size(class_i,2)
        switch class_i(j)
            case 1
                ...
                y(j,:) = yr(1, shift(j):shift(j)+length-1)*Acc(j);
...
x_train = x_train';
y_train = y_train';
save(strcat('../samples/te_3_',num2str(snr)),'x_train','y_train','Ac', 'snr','length','-v7.3')
end